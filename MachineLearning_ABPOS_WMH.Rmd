...
setwd("/Volumes/USC 2025/NGP/PM591/ML_ABPos_WMH_FinalProject")
...
- describe problem, modeling approaches, and results
- use 3 distinct, appropriate methods
- describe splitting of data into train/test and any resampling strategy like cross validation used for comparing models, tuning, or feature selection
...
- do the predictive models developed achieved sufficient accuracy to be useful for predicting new observations? this is based on 1. practical utility of the model, 2. strengths and limitations, and 3. possible improvements or extensions
...
table(CUdata$CDX_Cog)

# converting E4 status and Amyloid Positivity to single binary feature 
# otherwise ML models interpret E4 = 0 and E4 = 1 in same person (everyone has a value of E4- or E4+ = ex. they'll have "1" for the feature APOE4_Positivity.X0 if they're E4- but ALSO have "0" encoded for feature APOE4_Positivity.X1... so ML models might "double count")
CUdata$APOE4_Positive <- ifelse(CUdata$APOE4_Positivity == "1", 1, 0)
CUdata$X01_AB_FBB_AB_pos <- ifelse(CUdata$X01_AB_FBB_AB_pos == "1", 1, 0)
# scanner had extra empty string of "" w/no values, removing it
...
# recoding factors with 2 levels (binary) to numeric (ENet and XGBoost expect numeric input, not factors)
...
### 
# encoding categorical variable for linear model to prevent multicollinearity
# white == reference; adds "Ethnicity_Black" which == 1 if Black, 0 if not; also adds "Ethnicity_Hispanic" which == 1 if Hispanic, 0 if not 
# Ethnicity_White is implicitly encoded 
final_doc <- dummy_cols(final_doc, select_columns = "Ethnicity", remove_first_dummy = TRUE, remove_selected_columns = TRUE)
# drops original 'Ethnicity' column)
## sanity check (all should be FALSE)
sapply(final_doc, is.factor)
# z-scoring continuous vars
...
# removing Med_ID so it doesn't influence the model as a predictor -- identifiers are not be used as feature in ML models (unless your model is grouping) 
...
set.seed(101) 
#creates a sequence up to total # of rows in real dataset -- randomly selecting row #s to be stored in training
...
xgb_param_space <- ps(nrounds = p_int(50, 300), eta = p_dbl(0.01, 0.3),max_depth = p_int(2, 6))

# tuning my XGBoost model using inner 3 fold CV (using autotuner)
auto_xgb <- AutoTuner$new(learner = lrn_xgb,
  resampling = rsmp("cv", folds = 3), # inner CV
  measure = msr("classif.auc"), search_space = xgb_param_space, terminator = trm("evals", n_evals = 30), tuner = tnr("random_search"))
...
bmr <- benchmark(benchmark_grid(tasks = task, learners = learners, resamplings = cv10))
